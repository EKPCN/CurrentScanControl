...
# directory names
#**********************************************
datadirectory = "/home/readout/pXar_output/20150317_M0307_trimvcal40_p20/test"	#to complete
...
################################################################################################
desVoltage = 32;		#desired tube voltage in kV 				(2kV < x < 60kV and integer)
...
#port = serial.Serial('/dev/ttyS0', 9600, 8, 0, 1, 10)
...
print "This program allows to control the settings of the XrayTube"
print "***********************************************************"
print "Possible commands are"
print "setCurrent"
print "setVoltage"
print "openShutter int (int = 2 (back) or int = 3 (down)!)"
print "closeShutter int (int = 2 (back) or int = 3 (down)!)"
print "actSettings (prints the actual settings of the Xray tube)"
print "exit (the program will end, the tube is still running!)"

while(True):
	command = raw_input("Please type in a command: ")
	if(command == "setCurrent"):
		int_current_mA = input("Please choose the current (2mA < int_mA < 80mA and integer!) ")
		while(testVoltage(int_current_mA) == False):
			int_current_mA = input("Please choose a correct current value (2mA < int_mA < 80mA and integer!) ")
		setCurrent(int_current_mA)
	elif(command == "setVoltage"):
		int_voltage_kV = input("Please choose the voltage (2kV < int_kV < 60kV and integer!)")
		while(testVoltage(int_voltage_kV) == False):
			int_voltage_kV = input("Please choose a correct voltage value (2kV < int_kV < 60kV and integer!) ")
		setVoltage(int_voltage_kV)
	elif(command == "openShutter"):
		int_shutternumber = input("Please choose the shutter number (2 (back) or 3 (down)!) ")
		while(int_shutternumber != 2 and int_shutternumber != 3 or type(int_shutternumber) != int):
			int_shutternumber = input("Please choose a correct shutternumber to open (2 (back) or 3 (down)... ");
	...
	elif(command == "actSettings"):
		actSettings();
	elif(command == "exit"):
		print "The program will end, the tube is still running!!!";
		sys.exit();
	else:
		print "This command isn't known, please check your input"
